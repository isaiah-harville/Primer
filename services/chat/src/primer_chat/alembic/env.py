"""Alembic environment for the Chat schema.

Runs synchronously, and against its own schema only: Chat and Control share
a PostgreSQL instance but not a migration history, so neither service's
migration run touches the other's tables.
"""
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        connection.execute(text(f"CREATE SCHEMA IF NOT EXISTS {CHAT_SCHEMA}"))
        connection.commit()
        context.configure(
